...
class PointCloudVisualizer():
                ...
                for pcl in sensor_data:
                    if self.check_exists_or_new(robot_id, pcl.keyframe_id):
                        if pcl.keyframe_id in self.pose_graph_viz.robot_pose_graphs[robot_id]:                        
                            ...
                            tf_to_publish.header.stamp = self.node.get_clock().now().to_msg()
    ...
    def visualization_callback(self):
        ...
        # self.node.get_logger().info(f"Marker_array_size {len(self.markers_to_publish)} , tfs_array_size {len(self.tfs_to_publish)}")
        for pc in self.markers_to_publish:
            self.pointclouds_keys_published.add((pc.ns, pc.id))
            # self.node.get_logger().info("Publishing Markers")
            self.markers_publisher.publish(pc)
        for tf in self.tfs_to_publish:
            self.tf_broadcaster.sendTransform(tf)
            # self.node.get_logger().info("Publishing TF")
        self.markers_to_publish = []
        # self.node.get_logger().info(f"Pose-graph robots: {list(self.pose_graph_viz.robot_pose_graphs.keys())}"
